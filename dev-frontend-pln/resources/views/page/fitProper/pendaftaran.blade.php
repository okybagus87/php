...
@section('content')
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card">
                <form action="{{ route('fitProper.tambah') }}" method="POST">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group col-12">
                            <label for="exampleInputEmail1">Proyeksi Jabatan</label>
                            <select name="pj" class="form-control pj">
                                <option value=""></option>
                                @foreach ($jabatan->data as $data)
                                    <option value="{{ $data->attributes->namaJabatan }}">
                                        {{ $data->attributes->namaJabatan }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-12">
                            ...
                            <select name="jj" class="form-control jj">
                                <option value=""></option>
                                @foreach ($jabatan->data as $data)
                                    <option value="{{ $data->attributes->namaJabatan }}">
                                        {{ $data->attributes->namaJabatan }}
                        ...
                        <div class="form-group col-12">
                            <label for="exampleInputEmail1">Penguji</label>
                            <select name="penguji" class="form-control penguji" multiple>
                                <option value=""></option>
                                @foreach ($penguji->data as $data)
                                    <option value="{{ Crypt::encryptString($data->id) }}">
                                        {{ $data->attributes->id_penguji->data->attributes->namaPegawai }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
...
@section('script')
    ...
    <script>
        ...
        $('.pj').select2({
            placeholder: '-- Pilih Proyeksi Jabatan --',
            theme: 'classic'
        });
        $('.jj').select2({
